...
static bool mg_tcpip_driver_pico_w_init(struct mg_tcpip_if *ifp) {
  ...
  cyw43_wifi_ap_set_password(&cyw43_state, strlen(d->pass), d->pass);
  cyw43_wifi_ap_set_ssid(&cyw43_state, strlen(d->ssid), d->ssid);
...
static size_t mg_tcpip_driver_pico_w_tx(const void *buf, size_t len,
  ...
  return cyw43_send_ethernet(&cyw43_state, CYW43_ITF_AP, len, buf, false) ? 0 : len;
  (void) ifp;
}

static bool mg_tcpip_driver_pico_w_up(struct mg_tcpip_if *ifp) {
  return cyw43_wifi_link_status(&cyw43_state, CYW43_ITF_AP) == CYW43_LINK_DOWN ? 1 : 0;
}
