...
enum { STATUS_WAITING, STATUS_SUCCESS, STATUS_FAIL };
...
static int fetch_ota_metadata(struct mg_http_message *response) {
  ...
  if (!mg_json_get_num(response->body, "$.crc", &result)) goto fetch_failed;
  s_ota_metadata.crc = (uint32_t) result;
...
static void s_firmware_fn(struct mg_connection *c, int ev, void *ev_data) {
  ...
  if (ev == MG_EV_POLL) {
    ...
    if (s_ota_status == STATUS_FAIL) return;
    ...
    MG_DEBUG(("Connection closing, downloaded %ld/%ld bytes", ofs,
              s_ota_metadata.size));
    if (s_ota_status != STATUS_FAIL && ota_begun) {
...
void mg_ota_url_check(struct mg_mgr *mgr, const char *current_version,
  ...
  s_version_status = STATUS_WAITING;
  s_ota_status = STATUS_WAITING;
  ...
  while (s_version_status == STATUS_WAITING) mg_mgr_poll(mgr, 10);
  if (s_version_status == STATUS_SUCCESS) {
  ...
  while (s_ota_status == STATUS_WAITING) mg_mgr_poll(mgr, 10);
  if (s_ota_status == STATUS_FAIL && fn) fn("OTA fail");
  free_ota_metadata();
}
