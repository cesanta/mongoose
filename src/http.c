...
struct mg_connection *mg_http_listen(struct mg_mgr *mgr, const char *url,
                                     mg_event_handler_t fn, void *fn_data) {
	return mg_http_listen_on_socket(mgr, url, MG_INVALID_SOCKET, fn, fn_data);
}

struct mg_connection *mg_http_listen_on_socket(struct mg_mgr *mgr, 
      const char *url, MG_SOCKET_TYPE fd, mg_event_handler_t fn, void *fn_data) {
  struct mg_connection *c = mg_listen_on_socket(mgr, url, fd, fn, fn_data);
  if (c != NULL) c->pfn = http_cb;
  return c;
}
