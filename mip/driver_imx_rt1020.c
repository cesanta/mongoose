...
enum { RT1020_ADDR = 0x02, RT1020_PHY_BCR = 0, RT1020_PHY_BSR = 1 };     // PHY constants
...
static uint32_t rt1020_read_phy(uint8_t addr, uint8_t reg) {
  ENET->EIR |= BIT(23); // MII interrupt clear
...
static void rt1020_write_phy(uint8_t addr, uint8_t reg, uint32_t val) {
  ENET->EIR |= BIT(23); // MII interrupt clear
...
static bool mip_driver_imx_rt1020_init(struct mip_if *ifp) {
  ...
  rt1020_write_phy(RT1020_ADDR, RT1020_PHY_BCR, 0x8000); // PHY W @0x00 D=0x8000 Soft reset
  while (rt1020_read_phy(RT1020_ADDR, RT1020_PHY_BSR) & BIT(15)) {delay(1);} // Wait finished poll 10ms

  // PHY: Start Link
  {
    rt1020_write_phy(RT1020_ADDR, RT1020_PHY_BCR, 0x1200); // PHY W @0x00 D=0x1200 Autonego enable + start
    rt1020_write_phy(RT1020_ADDR, 0x1f, 0x8180);    // PHY W @0x1f D=0x8180 Ref clock 50 MHz at XI input

    uint32_t bcr = rt1020_read_phy(RT1020_ADDR, RT1020_PHY_BCR);
    bcr &= ~BIT(10); // Isolation -> Normal
    rt1020_write_phy(RT1020_ADDR, RT1020_PHY_BCR, bcr);
  }
...
// Up/down status
static bool mip_driver_imx_rt1020_up(struct mip_if *ifp) {
  uint32_t bsr = rt1020_read_phy(RT1020_ADDR, RT1020_PHY_BSR);
  (void) ifp;
