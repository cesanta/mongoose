...
static pio_sm_config piospi_init(PIO pio, uint sm, uint addr, uint data,
                                 uint clk) {
  ...
  pio_sm_set_pindirs_with_mask(pio, sm, (1 << data) | (1 << clk),
                               (1 << data) | (1 << clk));
...
void hwspecific_spi_init(void) {
  ...
  s_piospi_sm_config =
      piospi_init(SPIPIO, SPISM, s_piospi_sm_addr, DATAPIN, CLKPIN);
  mg_delayms(50);
}
...
void hwspecific_spi_txn(void *arg, uint8_t *txdata, uint8_t *rxdata,
                        size_t len) {
...
static const struct mg_tcpip_spi spi = {NULL, hwspecific_spi_begin,
                                        hwspecific_spi_end, hwspecific_spi_txn};
...
static const struct mg_tcpip_driver_cyw_firmware fw = {
    (const uint8_t *) w43439A0_7_95_49_00_combined,
    (size_t) CYW43_WIFI_FW_LEN,
    (const uint8_t *) wifi_nvram_4343,
    (size_t) sizeof(wifi_nvram_4343),
    (const uint8_t *) (w43439A0_7_95_49_00_combined +
                       sizeof(w43439A0_7_95_49_00_combined) - CYW43_CLM_LEN),
    (size_t) CYW43_CLM_LEN};
  ...
  switch (state) {
    ...
    case SCANNING:
      if (ev == MG_TCPIP_EV_WIFI_SCAN_RESULT) {
        ...
        // struct mg_tcpip_driver_cyw_data *d = (struct mg_tcpip_driver_cyw_data
        // *) ifp->driver_data;
...
static struct mg_tcpip_driver_cyw_data d = {
    {WIFI_SSID, WIFI_PASS, "mongoose", "mongoose", 0, 0, 0, 0, 10, true},
