...
int main(int argc, char *argv[]) {
  ...
  // Check if a systemd socket is available
  MG_SOCKET_TYPE socket_fd = MG_INVALID_SOCKET;
  for (int fd = SD_LISTEN_FDS_START; fd < SD_LISTEN_FDS_START + sd_listen_fds(0); fd++) {
    if (sd_is_socket_inet(fd, AF_INET, SOCK_STREAM, -1, 0))
    {
      socket_fd = fd;
      MG_INFO(("got socket on fd %i from systemd", fd));
      break;
    }
  }
  ...
  // Use the external socket function to provide a socket file descriptor
  // If the sockete file descriptor is MG_INVALID_SOCKET the socket will be created for us
  if ((c = mg_http_listen_external_socket(&mgr, s_addr2, socket_fd, cb, (void *) 1)) == NULL) {
    MG_ERROR(("Cannot listen on %s. Use http://ADDR:PORT or :PORT",
              s_addr2));
    exit(EXIT_FAILURE);
  }
