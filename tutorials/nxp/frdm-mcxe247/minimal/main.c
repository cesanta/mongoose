...
#define LED1 PIN('C', 13)  // Red
#define LED2 PIN('B', 11)  // Green
#define LED3 PIN('C', 12)  // Blue

static uint32_t s_rng_seed;

static void hal_rng_init(void) {
  s_rng_seed = hal_entropy_init();
}

static uint32_t hal_rng_read(void) {
  uint32_t seed = s_rng_seed, entropy;
  if (hal_entropy_get(&entropy)) {
    entropy *= 0x85ebca6b;
    entropy ^= entropy << 16;
    entropy ^= entropy >> 5;
    seed ^= entropy;
...
int main(void) {
  ...
  hal_gpio_write(LED1, true);
  hal_gpio_output(LED2);
  hal_gpio_write(LED2, true);
  hal_gpio_output(LED3);
  hal_gpio_write(LED3, true);
