...
int main(void) {
  ...
  // Mongoose event manager processes both Mongoose API and BSD sockets API
  struct mg_mgr mgr;
  mg_mgr_init(&mgr);

  // This is Mongoose-API HTTP server
  mg_http_listen(&mgr, "http://0.0.0.0", http_ev_handler, NULL);

  // This is BSD sockets API tcp echo server that handles multiple concurrent
  // connections. Note - baremetal implementation is non-blocking only.
  echo_init(1234);

  // Baremetal superloop. Each function in the loop must not block
  for (;;) {
    mg_mgr_poll(&mgr, 0);
    echo_poll();
    blink_task();
