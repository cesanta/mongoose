...
// 49) SYS_FREQUENCY <= 480 MHz; hclk = SYS_FREQUENCY / HAL_HPRE ; hclk <= 240 MHz;
...
static inline uint32_t hal_apb_freq(int bus) {  // 1 for APB1 or 2 for APB2
  uint32_t sysclk = 0, pll_src_freq = 0;
  uint32_t hsi_freq = 64000000UL, hse_freq = 8000000UL, csi_freq = 4000000UL;
  uint32_t hclk, hpre_div, apb_div, pllm, plln, pllp, tmp;

  // 1. Determine system clock source
  switch (RCC->CFGR & RCC_CFGR_SWS) {
    case RCC_CFGR_SWS_HSI: sysclk = hsi_freq; break;
    case RCC_CFGR_SWS_CSI: sysclk = csi_freq; break;
    case RCC_CFGR_SWS_HSE: sysclk = hse_freq; break;
    case RCC_CFGR_SWS_PLL1:
      pllm = ((RCC->PLLCKSELR & RCC_PLLCKSELR_DIVM1_Msk) >>
              RCC_PLLCKSELR_DIVM1_Pos);
      plln = ((RCC->PLL1DIVR & RCC_PLL1DIVR_N1_Msk) >> RCC_PLL1DIVR_N1_Pos) + 1;
      pllp = ((RCC->PLL1DIVR & RCC_PLL1DIVR_P1_Msk) >> RCC_PLL1DIVR_P1_Pos) + 1;
      if (pllm == 0U) pllm = 1U;

      switch (RCC->PLLCKSELR & RCC_PLLCKSELR_PLLSRC) {
        case RCC_PLLCKSELR_PLLSRC_HSI: pll_src_freq = hsi_freq; break;
        case RCC_PLLCKSELR_PLLSRC_CSI: pll_src_freq = csi_freq; break;
        case RCC_PLLCKSELR_PLLSRC_HSE: pll_src_freq = hse_freq; break;
        default: pll_src_freq = 0U; break;
      }
      sysclk = (pll_src_freq / pllm) * plln / pllp;
      break;
    default: sysclk = hsi_freq; break;
  }

  // 2. Compute HCLK (core bus clock) from HPRE prescaler
  tmp = (RCC->D1CFGR & RCC_D1CFGR_HPRE_Msk) >> RCC_D1CFGR_HPRE_Pos;
  hpre_div = tmp < 8U ? 1U : (1U << (tmp - 7U));
  hclk = sysclk / hpre_div;

  // 3. Select the appropriate APB prescaler (from D2CFGR)
  if (bus == 1)
    tmp = (RCC->D2CFGR & RCC_D2CFGR_D2PPRE1_Msk) >> RCC_D2CFGR_D2PPRE1_Pos;
  else
    tmp = (RCC->D2CFGR & RCC_D2CFGR_D2PPRE2_Msk) >> RCC_D2CFGR_D2PPRE2_Pos;

  apb_div = tmp < 4U ? 1U : (1U << (tmp - 3U));
  return hclk / apb_div;
}
...
static inline void hal_uart_init(USART_TypeDef *uart, unsigned long baud) {
  ...
  if (uart == USART1) freq = hal_apb_freq(2), RCC->APB2ENR |= BIT(4);
  if (uart == USART2) freq = hal_apb_freq(1), RCC->APB1LENR |= BIT(17);
  if (uart == USART3) freq = hal_apb_freq(1), RCC->APB1LENR |= BIT(18);
...
static inline void hal_ethernet_init(void) {
  ...
  for (size_t i = 0; i < sizeof(pins) / sizeof(pins[0]); i++) {
    hal_gpio_init(pins[i], HAL_GPIO_MODE_AF, HAL_GPIO_OTYPE_PUSH_PULL,
        HAL_GPIO_SPEED_INSANE, HAL_GPIO_PULL_NONE, 11);  // 11 is the Ethernet function
  }
