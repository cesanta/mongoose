 ...
 * a Media-Independent Interface (MII), or a Reduced Media-Independent Interface
 * (RMII). The EMAC can poll for PHY ports on 32 different addresses. Each of
 * the PHY ports shall be treated independently.
 *
 */
...
/* A generic user-provided function that reads from the PHY-port at 'xAddress'(
 * 0-based ). A 16-bit value shall be stored in
 * '*pulValue'. xRegister is the register number ( 0 .. 31 ). In fact all PHY
 * registers are 16-bit. Return non-zero in case the action failed. */
 ...
 * PHY-port at 'xAddress' ( 0-based ). xRegister is the register number ( 0 ..
 * 31 ). Return non-zero in case the action failed. */
...
/* Do not use auto negotiation but use predefined values from
 * 'pxPhyObject->xPhyPreferences'. */
